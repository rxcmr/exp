...
public class EX11 {
   ...
   private void init() {
      ...
      do {
         try (var rdr = new BufferedReader(new InputStreamReader(System.in))) {
   ...
   private void solve() {
      ...
      if (a == 0) {
         ...
         if (discriminant > 0) {
            var x_1 = (-b + sqrt(discriminant)) / (2 * a);
            var x_2 = (-b - sqrt(discriminant)) / (2 * a);
            out.printf("\nx\u2081 = %s, x\u2082 = %s%n", x_1, x_2);
            ...
            var realPart = -b / (2 * a);
            var imaginaryPart = sqrt(-discriminant) / (2 * a);
            simplify();
            double[]
               p = new double[] { realPart, imaginaryPart },
               f = new double[] { b / gcf,  -discriminant / gcf, (-2 * a) / gcf};
            out.println("\nNo real solutions or roots:");
            final var result = format("x\u2081 = %s + i%s, x\u2082 = %s - i%s", p[0], p[1], p[0], p[1]).replaceAll("(- -)", "+ ");
            final var alternativeResult = format("x\u2081 = %s + i\u221a%s / %s, x\u2082 = %s - i\u221a%s / %s", f[0], f[1], f[2], f[0], f[1], f[2]);
            out.println(result + "\nAlternative result:\n" + alternativeResult);
         }
      }
   }
